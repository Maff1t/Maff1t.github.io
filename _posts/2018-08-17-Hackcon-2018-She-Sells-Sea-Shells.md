...
```
Ok then what is the "small gift" that the program is giving us?
Let's open it in radare and this is the main function:
...
Ok, we can see that the address of the buffer (local_40h) is placed in rax. We can notice that it use the 
"lea" instruction instead of MOV, because it want to "Load Effective Address" of the buffer.
Than it move rax in rsi as second parameter for the printf, as the x64 calling convetion want (first parameter in rdi , then rsi, rdx, rcx and so on).

After that, it request, as the other challenges, an input that is managed using the gets () function, without the lenght control.

Than we can control the Return address because there are no canaries on the stack. Moreover the NX bit is 0, then the stack is executable.
It means that if we place a shellcode in the stack and redirect the program to that address (that "casually" is given by the program), it can be executed.

Now we have all to write the exploit, and this time I used PwnTools to send the exploit to the remote server:
